...
const OrderButton: React.FC<{id: number}> = ({id}) => {
  const {cart, placeInCart} = useContext(ShopContext)
  const [isRegistered, setIsRegistered] = useState<boolean>(false)
  const i = id - 1

  useEffect(() => {
    placeInCart(c => !c.some(item => item.id == id) ? [...c, {id, count: 0}] : c)
  ...
  if(cart[i]?.count > 0) {
    return (
      <div className="flex justify-around w-full">
        ...
        <p className="font-black w-20 text-center backdrop-brightness-75 bg-opacity-50">{cart[i]?.count}</p>
